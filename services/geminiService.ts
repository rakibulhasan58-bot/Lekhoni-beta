...
const IMAGE_MODEL_NAME = 'gemini-2.5-flash-image'; // Dedicated image model
...
export const generateImage = async (
  ...
  // Extremely strict prompt to prevent the model from returning text descriptions (Chatbot behavior)
  const imagePrompt = `[System: Render this scene visually. Output ONLY the image. No text.]
...
${contentStyle}`;
  ...
  const generateSingleImage = async (): Promise<string> => {
      try {
        ...
        // 1. Check for empty candidates (often filtered by safety settings before content generation)
        const candidate = response.candidates?.[0];
        if (!candidate) {
             throw new Error("No candidates returned. The request might have been blocked by safety filters.");
        }
        ...
        // 3. Iterate through parts to find the image
        ...
        // 4. Handle "Chatty" refusals (Hallucinations where model talks instead of drawing)
        if (refusalText) {
  ...
  // Sequential Execution Loop to handle Rate Limits (429) robustly
  // We start with a short delay to be "fast", but increase it if we hit limits.
  let interRequestDelay = 500; 

  for (let i = 0; i < count; i++) {
      let attempts = 0;
      let generated = false;
      const MAX_RETRIES = 4; // Allow more retries for persistence

      while (attempts < MAX_RETRIES && !generated) {
          try {
              ...
              if (isRateLimit) {
                  ...
                  console.warn(`Quota exceeded (429). Retrying image ${i+1}/${count} (Attempt ${attempts}) in ${waitTime/1000}s...`);
                  await delay(waitTime);
                  // Permanently increase the inter-request delay if we hit a limit, to avoid immediate subsequent failure
      ...
      // If successful, wait a bit before the next one to respect RPM
      if (generated && i < count - 1) {
          await delay(interRequestDelay); 
      }
  }
